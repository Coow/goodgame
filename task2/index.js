...
(async () => {
    ...
    .then(response => {
        ...
        $('.search_result_row').each((index, game) => {
            ...
            if (game_title.toLowerCase().includes('a')) {
                return;
            }
            ...
            if ($(game).find('.search_price').children().length == 0) {
                originalPrice = $(game).find('.search_price').text().replace(regex, '');
            } else {
                //If the length is anything else, do some logic for getting discount, price and discounted price
                originalPrice = $(game).find('.search_price span').text().replace(regex, '');

                discount = $(game).find('.search_discount').text().replace(regex, '');
